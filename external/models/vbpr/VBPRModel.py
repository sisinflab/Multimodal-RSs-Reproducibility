...
class VBPRModel(torch.nn.Module, ABC):
    def __init__(self,
        ...
        self.F = torch.nn.Embedding.from_pretrained(torch.tensor(multimodal_features, device=self.device, dtype=torch.float32), freeze=False)
    ...
    def train_step(self, batch):
        user, pos, neg = batch
        xu_pos, gamma_u, gamma_i_pos, theta_u, proj_i_pos = self.forward(inputs=(np.array(user), np.array(pos)))
        xu_neg, _, gamma_i_neg, _, proj_i_neg = self.forward(inputs=(np.array(user), np.array(neg)))

        maxi = torch.nn.functional.logsigmoid(xu_pos - xu_neg)
        loss = -torch.mean(maxi)
        reg_loss = self.l_w * (1 / 2) * ((gamma_u ** 2).sum() +
                                         (gamma_i_pos ** 2).sum() +
                                         (gamma_i_neg ** 2).sum() +
                                         (theta_u ** 2).sum() +
                                         (proj_i_pos ** 2).sum() +
                                         (proj_i_neg ** 2).sum()) / np.array(user).shape[0]
        loss += reg_loss
